...
import { genProductSkuByOptions } from '@/utils/attrs';
...
export default function ProductOptions({ onOpenSelected, selectedSkuImages }: { selectedSkuImages: SkuSeletedImageType; onOpenSelected?: (key: number) => void }) {
    ...
    useEffect(() => {
        setProductData((prev) => ({ ...prev, options: attrOptions.map((item, index) => ({ ...item, sort: index })) }));
        if (productId > 0) {
            return;
        }
        setTimeout(() => {
    ...
    const makeSkuList = (productId: number) => {
        ...
        const newSkuList = genSkuList.map((sku, index) => ({
            ...
            stored: 0,
            ukey: '',
            akey: attrOptions.map((opt) => sku[opt.option]).join('-'),
