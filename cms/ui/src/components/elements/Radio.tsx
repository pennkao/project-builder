...
const Radio = ({ id, name, value, checked, label, onChange, className = '', disabled = false }: RadioProps) => {
    return (
        <label
            htmlFor={id}
        ...
        >
            ...
            />
            <span
                className={`flex h-5 w-5 items-center justify-center rounded-full border-[1.25px] ${checked ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'} ${
